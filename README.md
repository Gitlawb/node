...
| `GITLAWB_ARWEAVE_GATEWAY_URL` | Arweave gateway base URL for the three-outcome probe and the public verify endpoint. Default `https://arweave.net`. |
| `GITLAWB_ARWEAVE_VERIFY_RATE_LIMIT` | Max `/api/v1/arweave/anchors/verify/{item_id}` requests per client IP per hour (route flood brake; the route can issue outbound gateway requests per call). 0 disables. Default 120. |
