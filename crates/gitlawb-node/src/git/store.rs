...
// ── Reflog landing proof (#26 split 1 round 4) ─────────────────────────
//
// The durable-outbox reconcile gate at `durable_outbox::reconcile_prepared_from_disk`
// requires REQUEST-SPECIFIC evidence that a `prepared` / `uncertain`
// row's transition actually landed on disk, not just that the current
// ref state matches the row's `new_sha`. Current state is not
// request-specific: a deletion of an already-missing ref, or two
// requests deleting the same ref, both satisfy the SHA match on the
// later-issued row. The reflog is the durable, request-specific
// record — git writes one entry per ref update under
// `core.logAllRefUpdates = always`, so a reflog entry whose
// (old_sha → new_sha) tuple matches the row and whose timestamp is
// at or after `row.created_at` is request-specific proof the row's
// transition is the one that produced the on-disk state.

/// Return true iff the reflog at `repo_path` has an entry for
/// `ref_name` whose `(old, new)` matches `(old_sha, new_sha)` and
/// whose timestamp is `>= since`. The call fails closed: a missing
/// reflog file, a malformed entry, an absent namespace, or a
/// non-`always` config all return `Ok(false)` rather than
/// `Err`, because we want a missing proof to leave the row in
/// `prepared`/`uncertain` for human-attended recovery, not to abort
/// the reconcile pass.
///
/// `old_sha` may be `ZERO_SHA` only for create-from-nothing pushes,
/// but `has_reflog_landing` treats it as an ordinary literal — git
/// logs the pre-update value verbatim, and a no-previous-ref push
/// simply never appears in the reflog (no prior entry to log), so
/// `Ok(false)` is the correct verdict for a create-from-nothing row
/// (it requires no recovery — the cert/anchor land on the live path).
///
/// For deletion rows (`new_sha == ZERO_SHA`), git's design does
/// NOT preserve the per-ref reflog past the deletion: a successful
/// `update-ref -d <ref>` removes the ref AND its
/// `logs/<ref>` file. There is no durable, request-specific
/// Git-side evidence the deletion actually landed once the ref is
/// gone — the absence of a ref can equally well describe a
/// deletion of an already-missing ref, a stale `old_sha` from an
/// aborted push, or a different request's deletion. The
/// reconciliation protocol the reviewer demanded is therefore
/// "deletion rows require human-attended recovery" rather than
/// "auto-promote from on-disk absence". This is the conservative
/// fix: a missing reflog for a deletion is the gate's "no proof"
/// path, not a bug. Production deletion recovery lives in the
/// startup-time audit path the operator runs by hand.
///
/// The implementation parses the on-disk reflog file directly
/// (under `<bare>/logs/<ref-name>`) rather than `git reflog show`:
/// the file format is stable and small
/// (`<old_sha> <new_sha> <name> <email> <unix_ts> <tz>\n<message>`),
/// parsing it does not require a child process, and the deletion
/// case (`update-ref -d`) deletes the ref's reflog file so
/// `git reflog show <ref>` fails with "ambiguous argument" — but
/// the file-not-found path is the gate's safe answer.
pub fn has_reflog_landing(
    repo_path: &Path,
    ref_name: &str,
    old_sha: &str,
    new_sha: &str,
    ...
    // P1 (reviewer-1/2 round 4): deletion rows are NEVER
    // auto-promotable from reflog evidence — the reflog file
    // for a deleted ref is gone. Returning `Ok(false)` here
    // forces the row to stay `prepared`/`uncertain` for
    // human-attended recovery. This is the contract the test
    // `reconcile_does_not_promote_stale_deletion` pins.
    if new_sha == ZERO_SHA {
        return Ok(false);
    }
    // Bare repos keep per-ref reflogs at `<bare>/logs/<ref-name>`
    // (with each path segment as its own directory). For
    // `refs/heads/main` the file is `logs/refs/heads/main`; for
    // `refs/tags/v1` it is `logs/refs/tags/v1`. The leading `refs/`
    // is preserved; only the slash separators become path separators.
    let mut log_path = repo_path.to_path_buf();
    log_path.push("logs");
    ...
    let content = match std::fs::read_to_string(&log_path) {
        Ok(s) => s,
        // A missing reflog file is "no proof", not Err. This is the
        // common case for refs/tags/ and the case where the
        // reflog-evidence gate is doing its job.
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => return Ok(false),
        Err(e) => return Err(e.into()),
    };

    for line in content.lines() {
        // The reflog line format is documented in `git-reflog(1)`:
        //   <old_sha> <new_sha> <name> <email> <timestamp> <tz>
        //   \t<subject>
        // The subject is on the same line separated by a tab from
        // the header; `split_whitespace` on the first six tokens
        // pulls the (old, new, ..., ts) tuple we need.
        let mut parts = line.split_whitespace();
        let entry_old = match parts.next() {
            Some(s) => s,
            None => continue,
        };
        let entry_new = match parts.next() {
            Some(s) => s,
            None => continue,
        };
        if entry_old != old_sha || entry_new != new_sha {
            continue;
        }
        // Skip the committer name + email; the 5th token is the
        // unix timestamp in seconds. A parse failure here is a
        // malformed reflog entry — treat as "no proof" rather than
        // Err so the row stays recoverable.
        ...
        let entry_dt = match chrono::DateTime::from_timestamp(ts, 0) {
            Some(dt) => dt,
            None => continue,
        };
        if entry_dt >= since {
...
#[cfg(test)]
mod reflog_tests {
    //! The reflog-proof helper is the new request-specific evidence
    //! gate the reviewer demanded. These tests pin the behavior on
    //! a real bare repo so a future refactor cannot silently weaken
    //! the contract.
    //!
    //! Each test names the contract in its assertion message; reverting
    //! the helper turns the named assertion red.

    use super::*;
    use crate::db::deterministic_id;

    /// Build a real bare repo with a known ref at a known SHA. The
    ...
    #[test]
    fn init_bare_sets_log_all_ref_updates_to_always() {
        // P1 (reviewer-2 round 4): without `core.logAllRefUpdates=always`
        // set by `init_bare`, tag pushes leave no reflog and the
        // reconcile gate can never promote a `refs/tags/*` row.
        ...
        assert!(
            ...
            "core.logAllRefUpdates must be set; git config --get returned non-zero"
        );
        let got = String::from_utf8_lossy(&value.stdout).trim().to_string();
        assert_eq!(
            got, "always",
            "init_bare must set core.logAllRefUpdates=always so refs/tags are logged; \
             got `{got}` — without `always`, tag pushes never produce a reflog and \
             reconcile cannot promote them"
        );
    }

    #[test]
    fn init_bare_logs_tag_pushes_under_always() {
        // Mirror the reviewer's reproduction: under `always`, an
        // `update-ref refs/tags/v1` produces `logs/refs/tags/v1`.
        let tmp = tempfile::tempdir().unwrap();
        let bare = tmp.path().join("repo.git");
        init_bare(&bare).unwrap();
        let sha = seed_bare_ref(&bare, "refs/tags/v1");
        // The reflog file should exist after `update-ref`.
        let tag_log = bare.join("logs/refs/tags/v1");
        assert!(
            tag_log.exists(),
            "init_bare must configure reflog for tags too; logs/refs/tags/v1 missing"
        );
        // `has_reflog_landing` returns true for the matching (0, sha) tuple.
        let old = "0".repeat(40);
        let since = chrono::Utc::now() - chrono::Duration::hours(1);
        let proof = has_reflog_landing(&bare, "refs/tags/v1", &old, &sha, since).unwrap();
        assert!(
            proof,
            "tag push must produce a reflog entry that the reconcile gate accepts"
        );
    }

    #[test]
    fn has_reflog_landing_returns_false_when_ref_has_no_log() {
        // A bare repo with no `git update-ref` ever performed on
        // `refs/heads/main` has no reflog file under `always` until
        // the first update. The helper must return Ok(false), not
        // Err, so reconcile treats this as "no proof" and leaves the
        // row recoverable.
        let tmp = tempfile::tempdir().unwrap();
        let bare = tmp.path().join("repo.git");
        init_bare(&bare).unwrap();
        let proof = has_reflog_landing(
            &bare,
            "refs/heads/main",
            &"0".repeat(40),
            &"a".repeat(40),
        ...
        assert!(
            !proof,
            "a ref with no reflog entry must return Ok(false) — reconciling it would \
             require exactly the request-specific proof this gate exists to demand"
        );
    }

    #[test]
    fn has_reflog_landing_returns_false_for_wrong_transition() {
        // The reflog has an entry for `old_a → new_a`. A reconcile
        // row with the wrong `new_sha` must NOT match — the gate is
        // request-specific, not just ref-presence.
        let tmp = tempfile::tempdir().unwrap();
        let bare = tmp.path().join("repo.git");
        init_bare(&bare).unwrap();
        ...
        assert!(
            !proof,
            "a reflog entry for `0 → {sha}` must not satisfy a row claiming `0 → deadbeef`"
        );
    }

    #[test]
    fn has_reflog_landing_respects_since_floor() {
        // A row's `created_at` is BEFORE the reflog entry's
        // timestamp — the entry cannot be the row's own transition.
        let tmp = tempfile::tempdir().unwrap();
        let bare = tmp.path().join("repo.git");
        init_bare(&bare).unwrap();
        let sha = seed_bare_ref(&bare, "refs/heads/main");
        ...
        assert!(
            ...
            "a reflog entry that is older than the row's `created_at` must not \
             satisfy the gate — that is exactly the wrong-pusher-identity attack the \
             gate exists to prevent"
        );
    }

    #[test]
    fn has_reflog_landing_does_not_promote_deletions() {
        // P1 (reviewer-1/2 round 4): git removes the per-ref reflog
        // when a ref is deleted, so there is no durable,
        // request-specific evidence a deletion actually landed. The
        // gate fails closed: `has_reflog_landing` returns `Ok(false)`
        // for any deletion row, forcing the row to stay
        // `prepared`/`uncertain` for human-attended recovery rather
        // than risking a cert issued under the wrong request identity.
        let tmp = tempfile::tempdir().unwrap();
        let bare = tmp.path().join("repo.git");
        init_bare(&bare).unwrap();
        let sha = seed_bare_ref(&bare, "refs/heads/main");
        ...
        assert!(
            !proof,
            "deletion rows must never satisfy the reflog gate — git's \
             design does not preserve per-ref reflogs past `update-ref -d`, \
             so absence of evidence is the only safe verdict"
        );
    }

    #[test]
    fn has_reflog_landing_keeps_helper_used() {
        // Reference to the deterministic_id helper to keep the
        // dev-dependency tree honest if the helper is ever moved to
        // a feature-gated module.
        let _ = deterministic_id(&["reflog-test"]);
    }
}
