...
mod tests {
    ...
    mod replay_guards {
        ...
        /// A non-default multibase spelling of the same `did:key`, built by hand so
        /// this test does not depend on how `Did` chooses to encode. Stays within
        /// `MAX_METHOD_ID_LEN` (base16 would not) while still decoding to the
        /// identical key via `to_verifying_key`.
        fn did_key_alternate_spelling(kp: &Keypair) -> String {
            let mut prefixed = vec![0xed, 0x01];
            prefixed.extend_from_slice(&kp.verifying_key().to_bytes());
            let encoded = multibase::encode(multibase::Base::Base32Lower, &prefixed);
        ...
        async fn the_identity_cap_is_keyed_on_the_resolved_key(pool: PgPool) {
            ...
            let did_b = did_key_alternate_spelling(&kp);
            ...
            let vk_b = did_b
                .parse::<Did>()
                .expect("alternate did parses")
                .to_verifying_key()
                .expect("alternate did resolves");
            assert_eq!(
                vk_a.to_bytes(),
                vk_b.to_bytes(),
